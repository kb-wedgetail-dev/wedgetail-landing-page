...
Hero placeholders are transparent WebP renders of the runtime model, produced with the same lighting and initial pose. `src/hero-poster.js` places them using the exported bounds and live content measurements before Three.js is ready. To regenerate after changing the model or hero pose, run the local server, open `/scripts/render-hero-posters.html`, and click **Render hero posters**. This writes the two images and `src/hero-poster-bounds.json` through a same-origin development-only endpoint. Rebuild and redeploy afterward.
